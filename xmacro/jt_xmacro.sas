 ...
 /*    DATA:                                                     */
 /*                                                              */
 /* SUPPORT: saswss(Warren Sarle)        UPDATE:  01Dec00        */
 /*     REF:                                                     */
 /*    MISC:                                                     */
 /*                                                              */
 /****************************************************************/
 ...
 * error checking (%xchk...)
										Line 842:  %macro xchkerr(_arg);
										Line 851:  %macro xchkech(_arg);
										Line 895:  %macro xchkdef(_arg,def);
										Line 910:  %macro xchkeq(_arg,def,noecho);
										Line 932:  %macro xchkone(_arg,def,noecho);
										Line 958:  %macro xchkuint(_arg,def);
										Line 986:  %macro xchkint(_arg,def,lb,ub);
										Line 1033: %macro xchknum(_arg,def,cond,noecho);
										Line 1072: %macro xchkmiss(_arg,def,cond);
										Line 1100: %macro xchkname(_arg,def);
										Line 1123: %macro xchkdsn(_arg,def,noecho);
										Line 1180: %macro xchkdata(_arg,def);
										Line 1222: %macro xchkkey(_arg,def,keys,delim);
										Line 1279: %macro xchklist(_arg,def,types,nmin,nmax);
										Line 1332: %macro xchkend(xdata);
										Line 1359: %macro xchkvar(data,by,varlists);
 * setting defaults (%xchk...)
 * using variable lists (%xvlist)
 * doing BY processing (%xbylist, %xdo_by)

There are macros to facilitate debugging and testing, and there are improved versions of 
		%SUBSTR (%xsubstr), 
		%SCAN (%xscan), 
		the MERGE statement(%xmerge), and the
		LAGn function (%xlag).
