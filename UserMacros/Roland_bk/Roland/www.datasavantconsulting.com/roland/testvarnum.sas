...
%put >>>> position of "dummy" var is %varnum(test,dummy) >>>>;
%put >>>> position of "num2" var is %varnum(test,num) >>>>;
%put >>>> position of non-existent var "roland" is %varnum(test,roland) >>>;
