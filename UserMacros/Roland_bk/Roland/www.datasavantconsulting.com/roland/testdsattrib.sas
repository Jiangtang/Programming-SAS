...
data template;
  ...
  aa='aa';
  bb=123;
  attrib aa informat=$char2. label='AA variable label'
         bb format=3. label='BB variable label'
         ;
run;
...
%dsattrib(template,rawds,refined)

options nocenter;
proc contents data=refined;
proc print data=refined;
