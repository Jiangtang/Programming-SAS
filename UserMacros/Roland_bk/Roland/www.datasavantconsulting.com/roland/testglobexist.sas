...
%macro testb;
  ...
  %let macc=cc;
  %put >>>>>> %globlist;
  %if %globexist(maca macb macc) %then %put >>> maca, macb, macc exist;
  %else %put >>>> maca macb macc do not all exist;

  %if %globexist(maca macb macxb) %then %put >>> maca, macb, macxb exist;
  %else %put >>>> maca macb macxb do not all exist;
%mend testb;
%testb
