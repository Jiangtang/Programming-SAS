...
proc format;
  value num 1='one' 2='two' 3='three';
  value $ch 'a'='AAAA' 'b'='BBBB' 'c'='CCCCC';
run;
...
data test2;
  num=3;char='b';output;
  format num num. char $ch.;
run;

%*- Note that the file specified below will become overwritten -;
%*- with the code required to recreate the formats. Make sure  -;
%*- you are not overwriting something you want to keep.        -;
%fmts2fda(work,file=fdafmts.sas)
