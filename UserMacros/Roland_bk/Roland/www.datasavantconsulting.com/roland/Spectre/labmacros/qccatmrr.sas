/*<pre><b>
/ Program   : qccatmrr.sas
/ Version   : 1.0
/ Author    : Roland Rashleigh-Berry
/ Date      : 26-Jan-2012
/ Purpose   : To QC the XLAB 2 CATMRR table
/ SubMacros : %mkformat %popfmt %qcworst %qcmean %qcmedian %qcrrcat: %hasvars
/             %match %varlist %nodup %words %varlen %attrv %vartype
...
/
/ Usage     : %qccatmrr(inlab=bilab2,inpopu=popu,popu=TS,instds=labstds,
/             intrt=gentrt,analno=3,repeat=worst,rrcat=-0.25/-1/1/4)
/===============================================================================
/ PARAMETERS:
/-------name------- -------------------------description------------------------
/ inlab             Input BILAB2-style dataset
/ rrcat             The RRCAT string (must be specified - see XLAB 2 manual)
...
%put MACRO CALLED: qccatmrr v1.0;


%macro qccatmrr(inlab=,
               repeat=WORST,
             multiple=CLOSEST,
               instds=,
                rrcat=-0.5 / -1 / 1 / 2,
  ...
  %local i j key spac err errflag repwidth workds rrcode rrlabels cond wherecls
         popudc anallbl savopts titlepos repno studies reptval maxrange misstxt
         studiesx
         ;
  %let err=ERR%str(OR);
  %let errflag=0;
  ...
  %if not %length(&inlab) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No lab dataset specified to inlab=;
  %end;

  %if not %length(&instds) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No lab standards dataset specified to instds=;
  %end;

  %if not %length(&intrt) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No gentrt dataset specified to intrt=;
  %end;

  %if not %length(&inpopu) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No population dataset specified to inpopu=;
  %end;

  %if not %length(&popu) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No population code specified to popu=;
  %end;

  %if not %length(&analno) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No analno specified to analno=;
  %end;

  %if not %length(&rrcat) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) No string specified to rrcat=;
  %end;
  ...
  %if not %length(&reptval) %then %do;
    %let errflag=1;
    %put &err: (qccatmrr) repeat=&repeat must be MEAN, MEDIAN, FIRST, LAST or WORST;
  %end;
  ...
  proc sql noprint;
    ...
    *- lab values -;
    create table _lab as (
      ...
      a.lab, a.labun, a.ll, a.ul, b.worstdir,
  ...
  *- all values including repeat values are used for this -;
  ...
  %qcrrcat(&rrcat,rrcode,rrlabels);
  %let maxrange=%words(&rrcode,delim=/);

  data _laball;
    set _linder _minmax _lablast2;
    lab=round(lab,0.0000000001);
    %do i=1 %to &maxrange;
      %if &i EQ 1 %then %let cond=if;
      %else %let cond=else if;
      &cond %scan(&rrcode,&i,/) then range&i=1;
    %end;
  run;
  ...
  *- we need to add some display and ordering variables for proc report -;
  ...
  proc summary nway missing data=_laball3(where=(_period in (1,2,3,4,5) &wherecls    and not _fgrept));
    ...
    var lab 
      ...
      %end;
      ;
    output out=_labsum(drop=_type_ _freq_) n(lab)=ntot 
      %do i=1 %to &maxrange;
        n(range&i)=n&i
      %end;
      ;
  ...
  data _labsum2;
    ...
    set _labsum;
    %do i=1 %to &maxrange;
      if n&i = 0 then n&i.str=put(n&i,4.);
      else n&i.str=put(n&i,4.)||" ("||put(100*n&i/ntot,5.1)||")";
    %end;
  run;
...
and specified markers - &popudc";
  ...
  *- use a repeated mid-line long hyphen (byte(131)) for the first footnote -;
  ...
  footnote2 "Key: [ or ] = include limit in category, ( or ) = exclude limit in category, LL = lower limit of normal, UL = upper limit of normal";
  footnote3 "N* = Number of patients with &misstxt per time-point or visit / summary visit";
  ...
  proc report missing headline headskip nowd split="@" spacing=5 data=_labsum3;  
    ...
    columns ("%sysfunc(repeat(%sysfunc(byte(131)),%eval(&repwidth-1)))"
              &trtvar _period visno _vistext ntot 
              %do i=1 %to &maxrange;
                n&i.str
              %end;
            );  
    ...
    define ntot / "N*" display;
    %do i=1 %to &maxrange;
      define n&i.str / display %scan(&rrlabels,&i,/)    width=13 ;
    %end;
  ...
  *- keep the _linder dataset if debug is set to yes -;
  ...
  %goto skip;
  %exit: %put &err: (qccatmrr) Leaving macro due to problem(s) listed;
  %skip:
