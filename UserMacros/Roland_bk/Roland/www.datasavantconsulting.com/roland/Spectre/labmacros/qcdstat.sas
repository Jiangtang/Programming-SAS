...
/ Date      : 26-Jan-2012
/ Purpose   : To QC the XLAB 2 DSTAT table
...
/
/             The post treatment values shown below the summary value are
/             selected according to the post treatment phase (atrcd="90000003")
/             and the post study phase (atrcd="90000011") for analno=1 but the
/             start date and start time is replaced by the on-treatment stop
/             date and stop time if this is later.
/
...
/
/ Usage     : %qcdstat(inlab=bilab2,inpopu=popu,popu=TS,instds=labstds,
...
/ gap=2             Spacing between groups of descriptive statistics columns
...
%macro qcdstat(inlab=,
  ...
  %local i j key spac err errflag style1 style2 workds repwidth startcol colw ncolw
         valtype popudc anallbl savopts titlepos repno maxvis
         labval labunit studies studiesx reptval wherecls misstxt
         ;
  %let err=ERR%str(OR);
  %let errflag=0;
  ...
  %if not %length(&inlab) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No lab dataset specified to inlab=;
  %end;

  %if not %length(&instds) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No lab standards dataset specified to instds=;
  %end;

  %if not %length(&intrt) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No gentrt dataset specified to intrt=;
  %end;

  %if not %length(&inpopu) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No population dataset specified to inpopu=;
  %end;

  %if not %length(&popu) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No population code specified to popu=;
  %end;

  %if not %length(&analno) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No analno specified to analno=;
  %end;

  %if not %length(&descstat) %then %do;
    %let errflag=1;
    %put &err: (qcdstat) No descriptive statistics number/labels specified to descstat=;
  %end;
  ...
  *- stage is reused to save processing time.      -;
  data _laball2;
    merge _labbl(keep=study ptno labnm _bl) _laball;
    by study ptno labnm;
  ...
  *- we need to add some display and ordering variables for proc report -;
  data _laball3;
    ...
    *- create phase variable for proc report skip control -;
    if _fgprev then _phase=1;
    else if _fgontv then _phase=2;
    else if _fgpostv then _phase=3;

    *- output value as _val -;
    _tp=1;
    _val=&labval;
    output;

    *- output change from baseline as _val -;
    if (_fgontv or _fgpostv) then do;
      _tp=2;
      _val=_chg;
      if not missing(_chg) then output;
    end;
  run;
  ...
  *- use "by" variable values in the titles -;
  title&titlepos "&acctype &repno  Descriptive statistics for &valtype values - &popudc";
  ...
  *- keep the _linder dataset if debug is set to yes -;
  proc datasets nolist;
    delete _lab _lab2 _lab3 _laball _laball2 _laball3 _bslval _lastval
  ...
  %goto skip;
  %exit: %put &err: (qcdstat) Leaving macro due to problem(s) listed;
  %skip:
