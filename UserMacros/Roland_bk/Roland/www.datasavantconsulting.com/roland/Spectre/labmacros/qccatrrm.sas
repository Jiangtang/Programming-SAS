/*<pre><b>
/ Program   : qccatrrm.sas
/ Version   : 1.0
/ Author    : Roland Rashleigh-Berry
/ Date      : 26-Jan-2012
/ Purpose   : To QC the XLAB 2 CATRRM table
...
/
/ Usage     : %qccatrrm(inlab=bilab2,inpopu=popu,popu=TS,instds=labstds,
...
%put MACRO CALLED: qccatrrm v1.0;


%macro qccatrrm(inlab=,
               repeat=WORST,
             multiple=CLOSEST,
               instds=,
                intrt=,
  ...
  %if not %length(&inlab) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) No lab dataset specified to inlab=;
  %end;

  %if not %length(&instds) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) No lab standards dataset specified to instds=;
  %end;

  %if not %length(&intrt) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) No gentrt dataset specified to intrt=;
  %end;

  %if not %length(&inpopu) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) No population dataset specified to inpopu=;
  %end;

  %if not %length(&popu) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) No population code specified to popu=;
  %end;

  %if not %length(&analno) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) No analno specified to analno=;
  %end;
  ...
  %if not %length(&reptval) %then %do;
    %let errflag=1;
    %put &err: (qccatrrm) repeat=&repeat must be MEAN, MEDIAN, FIRST, LAST or WORST;
  %end;
  ...
  *- all values including repeat values are used for this -;
  ...
  data _laball;
    set _linder _labbase _minmax _lablast2;
  run;
  ...
  *- stage is reused to save processing time.      -;
  data _laball2;
    ...
    by study ptno labnm;
    if _fgprev or _fgontv;

    n0=1;
    if rngflag="L" then n1=1;
    else if rngflag="N" then n2=1;
    else if rngflag="H" then n3=1;
    
    if _blflag NE " " then do;
      if _blflag NE "L" then n4=1;
      if _blflag NE "L" and rngflag EQ "L" then n5=1;
      if _blflag NE "H" then n6=1;
      if _blflag NE "H" and rngflag EQ "H" then n7=1;
    end;
  run;
  ...
  *- we need to add some display and ordering variables for proc report -;
  ...
  %let wherecls=and _hasbase and _haslast;
  %if &showmiss EQ Y %then %let wherecls=;

  proc summary nway missing 
    data=_laball3(where=((_fgprev or _fgontv or _fgpostv) &wherecls and not _fgrept));
    class labnm &trtvar _period visno cpevent;
    var n0 n1 n2 n3 n4 n5 n6 n7;
    output out=_labsum(drop=_type_ _freq_) n(n0)=ntot n(n1)=n1 n(n2)=n2 n(n3)=n3
    n(n4)=n4 n(n5)=n5 n(n6)=n6 n(n7)=n7;
  run;
  ...
  data _labsum2;
    length n0str n1str n2str n3str n4str n5str n6str n7str $ 13;
    set _labsum;
    n0str=put(ntot,4.);
    if n1=0 then n1str=put(n1,4.);
    else n1str=put(n1,4.)||" ("||put(100*n1/ntot,5.1)||")";
    if n2=0 then n2str=put(n2,4.);
    else n2str=put(n2,4.)||" ("||put(100*n2/ntot,5.1)||")";
    if n3=0 then n3str=put(n3,4.);
    else n3str=put(n3,4.)||" ("||put(100*n3/ntot,5.1)||")";

    if _period GT 1 then do;
      n4str=put(n4,4.);
      if n5=0 then n5str=put(n5,4.);
      else n5str=put(n5,4.)||" ("||put(100*n5/n4,5.1)||")";
      n6str=put(n6,4.);
      if n7=0 then n7str=put(n7,4.);
      else n7str=put(n7,4.)||" ("||put(100*n7/n6,5.1)||")";
    end;
   
    drop ntot n1 n2 n3 n4 n5 n6 n7;
...
based on difference from %lowcase(&bslnlbl) by visit - &popudc";
  ...
  *- use a repeated mid-line long hyphen (byte(131)) for the first footnote -;
  ...
  footnote3 "* %sysfunc(propcase(&patient))s with values >= LLN at Baseline, ** patients with values <= ULN at Baseline";
  footnote4 "N*** = Number of patients with &misstxt per time-point or visit / summary visit";
  footnote5 "Categorisation is based on original lab values.";
  footnote6 "The selected algorithm for repeat values is &repeat..";
  %if "&trialsft" EQ "Y" %then %do;
    %if %length(Trial(s): &studies) GT &repwidth %then %do;
      ...
      footnote7 "%scan(&studiesx,1,*)";
      footnote8 "%scan(&studiesx,2,*)";
      footnote10 &footnote;
    %end;
    %else %do;
      footnote7 "Trial(s): &studies";
      footnote9 &footnote;
    %end;
  %end;
  %else %do;
  ...
  proc report missing headline headskip nowd split="@" spacing=2 data=_labsum3;  
    by labgrp labgrpx labnmor labnm labnmx;
    columns ("%sysfunc(repeat(%sysfunc(byte(131)),%eval(&repwidth-1)))"
              &trtvar _period visno _vistext 
              ("_# &spat (%) with value at visit_" " " n0str n1str n2str n3str)
              ("&spat (%) with value at visit" "_compared to baseline_" n4str n5str n6str n7str)
            );  
    define &trtvar / order order=internal noprint;
    define _period / order order=internal noprint;
    define visno / order noprint;
    define _vistext / id order width=34 "Treatment/" "    Visit" spacing=0;
    define n0str / "N***" display         width=6;
    define n1str / display "    < LL"     width=13 ;
    define n2str / display "  [LL, UL]"   width=13 spacing=3;
    define n3str / display "     > UL"    width=13 ;
    define n4str / display "N at" "risk*" width=6  spacing=8;
    define n5str / display "    >= LL" "    to < LL" width=13;
    define n6str / display "N at" "risk**" width=6 spacing=3;
    define n7str / display "   <= UL" "   to > UL" width=13;
  ...
  *- keep the _linder dataset if debug is set to yes -;
  proc datasets nolist;
    delete _patinfo _patinfo2 _lab _lab2 _lab3 
           ...
           _labgrp _laball _laball2 _laball3 _labbase 
  ...
  %goto skip;
  %exit: %put &err: (qccatrrm) Leaving macro due to problem(s) listed;
  %skip:
