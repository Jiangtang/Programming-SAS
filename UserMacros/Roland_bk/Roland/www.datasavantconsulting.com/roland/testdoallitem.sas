...
data allchars;
  a='11.1';b='22.2';c='33.3';d='44.4';output;
  a='111';b='222';c='333';d='444';output;
run;

options mprint ;
data allnums;
  set allchars(rename=(%doallitem(%varlistc(allchars),'&item=char_&item ')))
;
%doallitem(%varlistc(allchars),'&item=input(char_&item,comma13.);')
drop %doallitem(%varlistc(allchars),'char_&item ')
;
run;

proc contents data=allnums;
proc print data=allnums;
run;
