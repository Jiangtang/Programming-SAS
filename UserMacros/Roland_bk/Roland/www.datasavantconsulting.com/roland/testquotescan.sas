...
%let str=%str(define string / display 'String' "variable's" "label" width=20 f=$20.);

%put >> %quotescan(&str,3) %quotescan(&str,2) %quotescan(&str,1) %quotescan(&str,99) >>;
