...
%put >>>> dataset creation datetime stamp is "%attrn(test,crdte)" >>>>;
%put >>>> dataset modification datetime stamp is "%attrn(test,modte)" >>>>;
%put >>>> number of observations is "%attrn(test,nobs)" >>>>;
%put >>>> number of logical observations is "%attrn(test,nlobs)" >>>>;
%put >>>> number of dataset variables is "%attrn(test,nvars)" >>>>;
