...
%put >>>> length of "dummy" var is "%varlen(test,dummy)" >>>>;
%put >>>> same again but no dollar thankyou is "%varlen(test,dummy,ugh)" >>>>;


%put >>>> length of "num" var is "%varlen(test,num)" >>>>;
%put >>>> "nodollar" options applied though not required is "%varlen(test,num,nodollar)" >>>>;
