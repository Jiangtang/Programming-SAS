...
%put MACRO CALLED: xlblocks v2.0;
...
%macro xlblocks(xlfile=,
  ...
  %if &readblocks EQ Y %then %do;
    ...
    %do i=1 %to &blocknobs;
      ...
      %*- character variables -;
      %let getn=%getvalue(_xlblocks,getnames,&i,usequotes=no);
      %let outl=%getvalue(_xlblocks,outlib,&i,usequotes=no);
      %let dsname=%getvalue(_xlblocks,dsname,&i,usequotes=no);
