...
%put MACRO CALLED: npctpvals v1.0;
...
%if not %length(&dsin) %then %do;
  %let error=1;
  %put ERROR: (npctpvals) dsin= not set for input data;
%end;

%if not %length(&byvars) %then %do;
  %let error=1;
  %put ERROR: (npctpvals) byvars= not set for by variables;
%end;

%if not %length(&trtvar) %then %do;
  %let error=1;
  %put ERROR: (npctpvals) trtvar= not set for treatment variable;
%end;

%if not %length(&respvar) %then %do;
  %let error=1;
  %put ERROR: (npctpvals) respvar= not set for response variable;
%end;
...
%if %length(&usetest) %then %do;
  ...
  and  "&usetest" NE "T2" %then %do;
    %let error=1;
    %put ERROR: (npctpvals) usetest= must be set to C, F, T2, T1L or T1R;
  %end;
%end;
...
%goto skip;
%error:
%put ERROR: (npctpvals) Leaving macro due to error(s) listed;
%skip:
%mend;
