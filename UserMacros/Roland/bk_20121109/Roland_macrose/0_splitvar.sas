...
/             inserted at the end. Searches for spaces and hyphens are only done
/             for half the column width so that the number of lines used for
/             overflow is kept to a limit.
/
...
/             first one.
/
/             This macro is a rewrite of the earlier versions of this name and
/             there is no backward compatibility possible.
/
...
columns term2;
define term2 / "SOC" "  Preferred Term" display flow width=&width;
run;
/===============================================================================
/ TEST OUTPUT FOLLOWS:
/===============================================================================
...
%put MACRO CALLED: splitvar v3.1;

%macro splitvar(oldvar,newvar,width,split=@,hindent=0,usecolon=yes,debug=n);
  ...
  length &lenvar __newstr __rest $ 500;
  ...
  if length(&oldvar) LE &width then __newstr=&oldvar;
    ...
    %if "&usecolon" NE "N" %then %do;
      if 0 LT index(left(__rest),": ") LE (&width*0.3) 
       then __hindent=index(left(__rest),": ")+1;
    %end;
    ...
    do while(__rest NE " ");
      ...
      if not __break then do;
        ...
        __rest=strip(substr(__rest,&width+1));
      end;
