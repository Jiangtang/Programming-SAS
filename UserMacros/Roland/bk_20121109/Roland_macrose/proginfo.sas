...
/              %lowcase %attrn %readpipe %layout2lsps
...
%put MACRO CALLED: proginfo v1.2;

%macro proginfo(program=,label=);

%local i holdlayout lsps;
...
%if %length(&_abort_) %then %do;
  %put ERROR: (proginfo) There has been an error in a previous macro so this macro will now exit;
  %goto error;
%end;
...
%if not %sysfunc(exist(&_ptlibref_..titles)) %then %do;
  %put ERROR: (proginfo) Titles information dataset "&_ptlibref_..titles" not found;
  %let _abort_=1;
  %goto error;
%end;
...
%if not %attrn(_titles,nobs) %then %do;
  %let _abort_=1;
  %put ERROR: (proginfo) No titles information found for program=&program label=&label;
  %goto error;
%end;
...
%if %length(&_pop_) %then %do;
  ...
  %if not %length(&_poplabel_) %then %do;
    %put ERROR: (proginfo) Population abbreviation "&_pop_" not mapped to a label;
    %let _abort_=1;
    %goto error;
  %end; 
%end;
...
%else %if %length(&_layout_) EQ %length(%sysfunc(compress(&_layout_,0123456789))) %then %do;
  ...
  and "&_layout_" NE "LT" and "&_layout_" NE "PT" %then %do;
    %put ERROR: (proginfo) Layout &holdlayout not recognised;
    %let _abort_=1;
    %goto error;
  %end;
...
%if not %length(&_layout_) %then %do;
  %put ERROR: (proginfo) Layout &holdlayout not mapped to one with a font size;
  %let _abort_=1;
  %goto error;
%end;
...
%if "&sysscp" EQ "WIN" %then %do;
  ...
  %if "&_ls_" EQ "ERROR" %then %do;
    %put ERROR: (proginfo) Layout &_layout_ not recognised by the "layout2lsps" macro;
    %let _abort_=1;
    %goto error;
  %end;  
...
%else %do;
  ...
  %let lsps=%readpipe(layout2lsps -l &_lmargin_ -r &_rmargin_ -t &_tmargin_ -b &_bmargin_ -p &_paper_ -f &_layout_);


  %*- check that the layout was OK for the layout2lsps script -;
  %if "%upcase(%substr(%quote(&lsps),1,5))" EQ "ERROR" %then %do;
    %put ERROR: (proginfo) Layout &_layout_ not recognised by the "layout2lsps" script;
    %let _abort_=1;
    %goto error;
  %end;
...
%if %length(&_ls_) LT 2 %then %do;
  %put ERROR: (proginfo) SAS linesize value could not be determined for layout &_layout_;
  %let _abort_=1;
  %goto error;
%end;
...
%goto skip;
%error:
%put ERROR: (proginfo) Leaving macro due to error(s) listed;
%skip:
%mend;
