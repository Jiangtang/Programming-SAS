...
/ msglevel=N        Message level to use inside this macro for information
/                   written to the log. Use msglevel=I for more information.
...
/                             will not use the normal labels.
...
/ hindent=1         The number of columns to leave blank for style=3 reports 
...
%put MACRO CALLED: npcttab v10.21;
...
%macro npcttab(dsin=,
  ...
  %if not %length(&msglevel) %then %let msglevel=N;
  %let msglevel=%upcase(&msglevel);
  %if "&msglevel" NE "N" and "&msglevel" NE "I" %then %let msglevel=N;

  %let savopts=%sysfunc(getoption(msglevel,keyword)) %sysfunc(getoption(mprint));
  ...
  %if %length(&dsparam) %then %do;
    ...
    *-- handle possible dataset modifiers --;
    ...
    %if %nlobs(_dsparam) NE 1 %then %do;
      %let errflag=1;
      %put &err: (ncpttab) The parameter dataset dsparam=&dsparam should have one;
      %put &err: (ncpttab) observation but this dataset has %nlobs(_dsparam) observations.;
      %put &err: (ncpttab) Checking of this dataset will continue but it can not be used.;
      %put;
    %end;
    ...
    %if %length(&varlist2) %then %do;
      %let errflag=1;
      %put &err: (ncpttab) Numeric variables are not allowed in the parameter dataset ;
      %put &err: (ncpttab) dsparam=&dsparam but the following numeric variables exist:;
      %put &err: (ncpttab) &varlist2;
      %put;
    %end;

    %if %varnum(_dsparam,dsparam) %then %do;
      %let errflag=1;
      %put &err: (ncpttab) The variable DSPARAM is present in the parameter dataset;
      %put &err: (ncpttab) dsparam=&dsparam but use of this variable inside a;
      %put &err: (ncpttab) parameter dataset is not allowed.;
      %put;
    %end;
    ...
    %if %length(&badvars) %then %do;
      %let errflag=1;
      %put &err: (ncpttab) The following list of variables in dsparam=&dsparam;
      %put &err: (ncpttab) do not match any of the macro parameter names so the;
      %put &err: (ncpttab) parameter dataset will not be used:;
      %put &err: (ncpttab) &badvars;
      %put;
    %end;
    ...
    *- the parameter dataset is good so call symput all the variables -;
    ...
    %put MSG: (ncpttab) The following macro parameters and their values were;
    %put MSG: (ncpttab) set as the result of use of the dsparam=&dsparam;
    %put MSG: (ncpttab) parameter dataset:;
    %mvarvalues(&varlist2);
    %put;
  ...
  %if not %length(&hindent) %then %let hindent=1;
