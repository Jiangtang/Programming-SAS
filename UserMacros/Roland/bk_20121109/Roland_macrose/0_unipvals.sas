...
/ SubMacros    : %words %qdequote %nobs %uniquad
...
/                      parameters added to pass to %uniquad. Major changes,
/                      hence (v2.0)
...
%put MACRO CALLED: unipvals v6.0;
  ...
  %if "&type" EQ "C" %then %do;
    ...
    *- for a by group then there is no point in calculating a p-value. -;
    ...
    %else %if "&usetest" EQ "F" %then %do;     %*- Fisher exact -;
     ...
     %fisher(dsin=_pvaldsin,dsout=_temp);
    ...
    %else %if "&usetest" EQ "C" %then %do;     %*- Chi-square -;
      ...
      %chisq(dsin=_pvaldsin,dsout=_temp);
    ...
    %else %if %length(&adjcntrvar) %then %do;     %*- CMH test -;
      ...
      %cmh(dsin=_pvaldsin,dsout=_temp);
    ...
    %uniquad(_temp#0#1#put(_statvalue,&statvalfmt));
    %uniquad(_temp#1#1#trim(put(_pvalue,&pvalfmt))||pvalid);
  ...
  %else %if "&type" EQ "N" %then %do;
    ...
    %if "&usetest" EQ "N" %then %do;  %*- non-parametric test -;
      ...
      *- merge with by values if any -;
      ...
      %uniquad(_temp#0#input("%upcase(&npvalstat)",_stator.)#put(_statvalue,&statvalfmt));
      %uniquad(_temp#1#input("%upcase(&npvalstat)",_stator.)#trim(put(_pvalue,&pvalfmt))||nparid);
...
%put NOTE: (unipvals) PROC &modelproc will use means statement "means &trtvar / hovtest=&hovtest glmwelch;" %eqsuff(&byvars);
  ...
  /*          %end; */
      ...
      %if not %length(&adjcntrvar) and not %length(&model) and "&welch" EQ "Y" and &hovsig %then %do;
        ...
        %uniquad(_temp#0#input("%upcase(&npvalstat)",_stator.)#put(_statvalue,&statvalfmt));
        %uniquad(_temp#1#input("%upcase(&npvalstat)",_stator.)#trim(put(_pvalue,&pvalfmt))||pvalid);
