...
/                For %box you can only specify a fill color and not a fill
/                pattern as the box shape used is assumed to be too small for
/                a fill pattern to be easily visible.
/
...
/
/                If you define fillcolor="white" then the first draw of the
/                shape is not done (unless for %box).
/
...
%put MACRO CALLED: rannomac v1.7;
...
%macro box(x=,
   ...
   linewidth=1,
    ...
    html=&html;
    x=&x-&width/2;y=&y-&height/2;function="poly";color=&fillcolor;style="msolid";line=1;
    size=&linewidth;output;
    html=' ';
...
%macro rod(y=,
 ...
 fillpattern="mempty");
  ...
  %*- skip the first time draw if the fill color is white -;
  %if not %index(%upcase(&fillcolor),WHITE) %then %do;
...
%macro larrow(y=,
  ...
  %if not %index(%upcase(&fillcolor),WHITE) %then %do;
    *- First time draw using "fillcolor" -;
    html=' ';
    x=&x1;y=&y;function="poly";color=&fillcolor;style="msolid";size=&linewidth;line=1;output;
