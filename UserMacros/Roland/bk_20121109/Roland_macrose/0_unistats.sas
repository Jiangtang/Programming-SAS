...
/ Notes        : BE WARNED that some major changes are being worked on with the
/                %unipvals macro and that the macro will be unstable for a while.
/
...
/ nparvarlist       Variables to be process by "proc npar1way"
...
/                   ------------------------------------------------------------
/ descstats         List of descriptive statistic labels separated by spaces
...
/                   paired statistics. Note that for paired statistics you might
/                   have to choose a value for mincolw= to prevent truncation of
/                   the second displayed value or you could increase the length
/                   of the format assigned to catnfmt= . This is explained in
/                   the notes below.
...
%put MACRO CALLED: unistats v10.0;
...
%macro unistats(dsin=,
  ...
  %local i err wrn errflag j pg allvars var usefmtord fmt ls trtvartype libname
         memname plug trtformat statfmts stat dpadj minmaxadj lowcase varlist2
         usetest exact strlen pctnfmt varlist3 varlist4 varlist5 varlistx
  ...
  %let savopts=%sysfunc(getoption(msglevel,keyword));
  ...
  %if %length(&descstats) %then %do;
    %let _statkeys_=%unimap(&descstats);
    %if not %length(&_statkeys_) %then %goto exit;
    %put NOTE: (unistats) The following labels defined to descstats: &descstats;
  ...
  %macro num(var,varnum,page,dpadj,minmaxadj);
    ...
    %if %index(%quotelst(%upcase(&statvarlist)),"%upcase(&var)") %then %do;
      ...
      %unipvals(dsin=__pin(where=(&trtvar &statvaltrtlist)),varname=&var,
              ...
              class=&class,model=&model,means=&means,
              lsmeans=&lsmeans,odstables=&odstables,weight=&weight,
  ...
  *- get rid of the append base datasets if they exist -;
  ...
  %if %length(&statvarlist) %then %do;
    data _null_;
      ...
      call symput(_pvalmacvar,compress(STAT1,"&fisherid.&chisqid.&cmhid.&anovaid.&welchid.&nparid"));
    run;
      ...
      if _vartype="C" then do;
        ...
        ************/
            if _dpadj=" " then do;
              ...
              else if _statname=:"PROB" then _str=put(_value,&probfmt);
    ...
    data &dstrantrt;
      ...
      %do i=1 %to &_trtnum_;
        %scan(&_trtvarlist_,&i,%str( ))=
          "%sysfunc(put&trtvartype(%qdequote(%scan(&_trtinlist_,&i,%str( ))),&trtfmt))"
      %end;
      ;
    run;
