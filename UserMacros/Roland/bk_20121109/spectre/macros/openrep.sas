...
/                temporary file.
/ SubMacros    : %endwith %readpipe %dequote. Relies on your having already
...
%if %length(&_abort_) %then %do;
  %put ERROR: (openrep) There has been an error in a previous macro so this macro will now exit;
  %goto error;
%end;
...
%if not %length(&_repid_) %then %do;
  %put ERROR: (openrep) You need to run the "titles" macro before calling this macro;
  %let _abort_=1;
  %goto error;
%end;
...
%goto skip;
%error:
%put ERROR: (openrep) Leaving macro due to error(s) listed;
%skip:
%mend;
