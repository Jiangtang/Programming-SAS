...
%global _ls_ _ps_;
%local error orient lspac psz pos ;
%let error=0;
...
%if not %length(&lmargin) %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Left margin in inches not supplied to lmargin=;
%end;

%if not %length(&rmargin) %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Right margin in inches not supplied to rmargin=;
%end;

%if not %length(&tmargin) %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Top margin in inches not supplied to tmargin=;
%end;

%if not %length(&bmargin) %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Bottom margin in inches not supplied to bmargin=;
%end;

%let paper=%upcase(&paper);
%if "&paper" NE "A4" and "&paper" NE "LETTER" and "&paper" NE "A4LETTER" %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Only paper=A4, paper=Letter or paper=A4Letter allowed 
paper=&paper;
%end;
...
%if "&orient" NE "L" and "&orient" NE "P" %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Layout= value must start with a "L" or "P";
%end;

%let pos=%verifyb(&layout,.0123456789);
%if &pos GT 3 %then %do;
  %let error=1;
  %put ERROR: (layout2lsps) Layout not recognised for layout=&layout;
%end;
...
%else %if &pos GT 1 %then %do;
  ...
  %if "&lspac" NE "C" and "&lspac" NE "CT" and "&lspac" NE "CW" and 
    ...
    %let error=1;
    %put ERROR: (layout2lsps) Line spacing not recognised for layout=&layout;
  %end;
%end;
...
%if "&psz" NE "7" and "&psz" NE "7.5" and "&psz" NE "8" and "&psz" NE "8.5" and 
  ...
  %let error=1;
  %put ERROR: (layout2lsps) Point size not 7-12 (by 0.5) for layout=&layout;
%end;
...
%error:
%*- set to impossible values for error condition -;
%let _ls_=ERROR;
%let _ps_=0;
%put ERROR: (layout2lsps) Leaving macro due to error(s) listed;
