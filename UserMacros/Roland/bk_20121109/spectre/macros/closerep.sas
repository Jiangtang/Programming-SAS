...
/ SubMacros    : %endwith %readpipe %dequote %pagexofy (or another macro
...
%put MACRO CALLED: closerep v4.3;

%macro closerep;

%local i donelist outdir outfile outlis outps;

%*- abort check -;
%global _abort_;
%if %length(&_abort_) %then %do;
  %put ERROR: (closerep) There has been an error in a previous macro so this macro will now exit;
  %goto error;
%end;
...
%if "&sysscp" EQ "WIN" %then %do;
  %let outdir=%dequote(%readpipe(echo '%OUTDIRWIN%'));
...
%goto skip;
%error:
%put ERROR: (closerep) Leaving macro due to error(s) listed;
%skip:
%mend;
