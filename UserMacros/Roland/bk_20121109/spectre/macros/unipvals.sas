...
/                or no test are used (the latter for 2x1 tables where
...
%put MACRO CALLED: unipvals v1.29;
...
%macro cmh(dsin=,dsout=);
  ...
  *- merge with by values -;
  ...
  %if %nobs(&dsin(where=(_respcnt>1))) %then %do;

  /* re-route to statsopdest if set */
    ...
    proc freq data=&dsin(where=(_respcnt>1));
  ...
  *- for a by group then there is no point in calculating a p-value. -;
  ...
  %if not %length(&usetest) and not %length(&adjcntrvar) %then %do;
    ...
    data _whichtest(keep=&byvars &byvars2 _test);
      ...
      set _expected(where=(_respcnt>1));
      by &byvars &byvars2;
...
_test="FISHER";
    ...
    data _notest(keep=&byvars &byvars2 _test);
      retain _test "NOTEST  ";
      set _expected(where=(_respcnt<2));
    run;
