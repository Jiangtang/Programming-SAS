...
%let paramlist=%mvarlist(npcttab);
...
%if %length(&dsparam) %then %do;
  ...
  *- the parameter dataset is good so call symput all the variables -;
  ...
  %put NOTE: (npcttab) The following macro parameters and their values were;
  %put NOTE: (npcttab) set as the result of use of the dsparam=&dsparam;
  %put NOTE: (npcttab) parameter dataset:;
  %mvarvalues(&varlist2);
  %put;
...
data &dsout;
  %if &style EQ 3 %then %do;
    length _combtext $ 256; *-was %eval(&ls-&trtwidth) -;
  %end;
  length &_trtvarlist_ 
...
%macro _npctrep(topline=,trtlabel=);

  proc report nowd missing headline &headskip split="&split" spacing=&spacing
  &spanrows data=&dsout

    style(COLUMN)={font_face=&font_face_stats font_weight=&font_weight_stats HTMLSTYLE="mso-number-format:'\@'"}
    style(LINES)={font_face=&font_face_other font_weight=&font_weight_other 
  font_style=&font_style_other HTMLSTYLE="mso-number-format:'\@'"}
...
%if "&print" EQ "Y" %then %do;
  %_npctrep(topline=&topline,trtlabel=&trtlabel);
  ods listing close;
%end;
...
&odsprerepcode
  ...
  %_npctrep(topline=N,trtlabel=&odstrtlabel);
