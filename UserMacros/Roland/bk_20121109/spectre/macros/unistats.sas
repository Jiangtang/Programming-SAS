...
/ Notes        : The formats nfmt=, minfmt= etc. will be applied to all numeric
...
/                   interaction.
/ statsopdest       Default is not to write the stat output to any destination.
...
/ odstblname        ODS tablename statement to store a dataset; per default,
/                   _odstablname will be used as output dataset name; multiple
/                   tablenames can be given, separated by spaces.
...
%put MACRO CALLED: unistats v6.23;
...
%let paramlist=%mvarlist(unistats);
...
%local i error j pg allvars var usefmtord fmt ls trtvartype libname memname plug
       ...
       strlen pctnfmt varlist3 varlist4 varlist5 forcenum denomsortvars badvars;
...
%if %length(&dsparam) %then %do;
  ...
  %if %nlobs(&dsparam) NE 1 %then %do;
    %let error=1;
    %put ERROR: (unistats) The parameter dataset dsparam=&dsparam should have one;
    %put ERROR: (unistats) observation but this dataset has %nlobs(&dsparam) observations.;
    %put ERROR: (unistats) Checking of this dataset will continue but it can not be used.;
    %put;
  %end;
  ...
  %if %length(&varlist2) %then %do;
    %let error=1;
    %put ERROR: (unistats) Numeric variables are not allowed in the parameter dataset ;
    %put ERROR: (unistats) dsparam=&dsparam but the following numeric variables exist:;
    %put ERROR: (unistats) &varlist2;
    %put;
  %end;

  %if %varnum(&dsparam,dsparam) %then %do;
    %let error=1;
    %put ERROR: (unistats) The variable DSPARAM is present in the parameter dataset;
    %put ERROR: (unistats) dsparam=&dsparam but use of this variable inside a;
    %put ERROR: (unistats) parameter dataset is not allowed.;
    %put;
  %end;

  proc contents noprint data=&dsparam out=_unicont(keep=name);
  run;
  ...
  data _null_;
    ...
    name=upcase(name);
    if name not in (%quotelst(&paramlist)) then badvars=trim(badvars)||" "||name;
    if last then call symput('badvars',trim(left(badvars)));
  run;

  %if %length(&badvars) %then %do;
    %let error=1;
    %put ERROR: (unistats) The following list of variables in dsparam=&dsparam;
    %put ERROR: (unistats) do not match any of the macro parameter names so the;
    %put ERROR: (unistats) parameter dataset will not be used:;
    %put ERROR: (unistats) &badvars;
    %put;
  %end;
  ...
  *- the parameter dataset is good so call symput all the variables -;
  ...
  %put NOTE: (unistats) The following macro parameters and their values were;
  %put NOTE: (unistats) set as the result of use of the dsparam=&dsparam;
  %put NOTE: (unistats) parameter dataset:;
  %mvarvalues(&varlist2);
  %put;
...
%macro num(var,varnum,page,dpadj,minmaxadj);
  ...
  %if %index(%quotelst(%upcase(&pvarlist)),"%upcase(&var)") %then %do;
    ...
    %unipvals(dsin=__pin(where=(&trtvar &pvaltrtlist)),
            ...
            glmlsmeans=&glmlsmeans,odstblname=&odstblname,glmweight=&glmweight,
            byvars=&byvars,byvars2=_page _varord _vartype _varname _varlabel _dpadj _minmaxadj,
            misspct=&misspct);
    proc append base=&dspout data=__pout;
    run;
...
%if %length(&dstrantrt) %then %do;
  ...
  %if %length(&pvarlist) and not %length(&byvars) %then %do;
...
%if "&unicatrep" EQ "Y" %then %do;
  ...
  %unicatrep(dsin=&dstrantrt,byvars=&byvars,odstrtlabel=&odstrtlabel,
  ...
  odsprerepcode=&odsprerepcode,odspostrepcode=&odspostrepcode,
  tfmacro=&tfmacro,odstfmacro=&odstfmacro);
