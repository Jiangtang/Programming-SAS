...
%macro mvarlist(scopename,contents);
  ...
  %if not %index(ASE,&contents2) %then %do;
    %put ERROR: (mvarlist) Expected "any", "empty" or "set" but you supplied "&contents";
    %goto error;
  %end; 

  %let dsid=%sysfunc(open(sashelp.vmacro(where=(scope="&scopename")),is));
  %if &dsid EQ 0 %then %do;
    %put ERROR: (mvarlist) sashelp.vmacro not opened due to the following reason:;
    %put %sysfunc(sysmsg());
    %goto error;
  %end;
  %else %do;
...
&mvarlist
  ...
  %goto skip;
  %error:
  %put ERROR: (mvarlist) Leaving mvarlist macro due to error(s) listed;
  %skip:
%mend;
