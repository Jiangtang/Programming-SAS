...
/ SubMacros    : %readpipe %dequote %qtrim %verifyb
...
/                returned.
/ Usage        : %let newvar=%getenv(uservar);
/ 
/===============================================================================
/ PARAMETERS:
/-------name------- -------------------------description------------------------
...
%if "&sysscp" EQ "WIN" %then %do;
  %let tempres=%readpipe(echo %nrbquote(%str(%%)%superq(envvar)%str(%%)));
  %*- if it did not resolve then set to null -;
  %if "%qsubstr(%superq(tempres),1,1)" EQ "%" %then %let tempres=;
%end;
%else %let tempres=%readpipe(echo $&envvar);
