...
%macro fmm;
  ...
  %if not %sysfunc(fileexist(%superq(act))) %then %do;
    %let error=1;
    %put ERROR: (fmm) "Actual" file %superq(act) does not exist;
    %put;
  %end;

  %if not %sysfunc(fileexist(%superq(exp))) %then %do;
    %let error=1;
    %put ERROR: (fmm) "Expected" file %superq(exp) does not exist;
    %put;
  %end;
  ...
  %if not &error %then %do;
    ...
    data _null_;
      ...
      if fid1<=0 then do;
        compare="DIFF";
        put 'ERROR: (fmm) "Expected" file could not be opened';
      end;
      fid2=fopen('fref2',"I",32767,"B");
      if fid2<=0 then do;
        compare="DIFF";
        put 'ERROR: (fmm) "Actual" file could not be opened';
      end;
