...
%macro backwards(len=200,var_in=coval,var_out=coval1,temp=temp1);
...
%macro split(len=200,ds_in=,ds_out=,var_in=);
...
data &ds_out;
    ...
    if len<=&len then &var_in.1=&var_in;   
        ...
        %do j=1 %to &bin; 
            %let jj=%eval(1+&j);
            %backwards(len=200,var_in=temp&j, var_out=&var_in.&j,temp=temp&jj);
        %end;
    end;    
