...
%*
%*  VERSION: 9.1.3
%*    28SEP2005 - Added support for Excel DATETIME data types
%*
%*********************************************************************;
...
else if lowcase(Type) EQ 'datetime' then do;
  ColType(ColumnIndex) = 'datetime';
  ColLen(ColumnIndex)  = max(ColLen(ColumnIndex), length(Value));
end;
else
...
do i = 1 to NCols;
  ...
  if lowcase(ColType(i)) EQ 'string'
    then put '$' ColLen(i) +(-1) ';';
    else if lowcase(ColType(i)) EQ 'datetime'
    then put '8 format=datetime.;';
    else put '8;';
end;
...
if (lowcase(ColType(1)) EQ 'string')
  ...
  else if (lowcase(ColType(1)) EQ 'datetime')
    then put '    then ' ColName(1) '= input(Value, anydtdtm.);';
...
do i = 2 to NCols;
  ...
  if (lowcase(ColType(i)) EQ 'string')
    then put '    then ' ColName(i) '= Value;';
      ...
      then put '    then ' ColName(i) '= input(Value, anydtdtm.);';
